...
async function onEditClick(event) {
 ...
 try {
   const response = await axios.get(`${API_BASE_URL}/api/users/form/${formId}`);
...
async function onDeleteClick(event) {
  ...
  try {
    const response = await axios.delete(`${API_BASE_URL}/api/users/form/${formId}`, {data: {user_id: user.id} });
...
form.addEventListener('submit', async (event) => {
  ...
  try {
    if (editingFormId) {
      const response = await axios.patch(`${API_BASE_URL}/api/users/form/${editingFormId}`, requestData);
      alert("Your request was updated successfully!");
    } else {
      const response = await axios.post(`${API_BASE_URL}/api/users/form`, requestData);
